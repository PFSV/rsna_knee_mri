...
PROMPT_VERSION = "v5.0-direct-evidence-only"
...
TARGET_POLICIES = {
    ...
    "Lateral OA": "direct_report_only",
    "PF OA": "direct_report_only",
    "Effusion": "direct_report_only",
    "Synovitis": "direct_report_only",
    "Baker's": "direct_report_only",
    "Contusion": "direct_report_only",
    "Fracture": "direct_report_only",
}
...
- ACL: anterior cruciate ligament injury. A directly asserted sprain, injury, partial tear,
  ...
  not positive unless the report calls it an injury. A suspected/possible injury is uncertain.
...
- Medial Meniscus: medial meniscus tear. A directly scoped normal meniscus or no-tear statement is
  negative. Degeneration or intrasubstance signal without a surface-reaching tear is not positive.
...
- Medial OA: osteoarthritis of the medial tibiofemoral compartment. Explicit OA/arthrosis is
  ...
  without an OA diagnosis is not positive.
...
- Synovitis: inflammation of the synovial lining. Explicit synovitis or language such as
  ...
  scoped normal synovium is negative. Nonspecific synovial thickening alone is not positive.
...
- Contusion: bone contusion / bone bruise only. Muscle or soft-tissue contusion does not count.
  ...
  contusion/bruise assertion is not positive.
...
Analyze only the supplied report text. Classify each target using direct statements about that
target. Do not infer one target from any other finding or target.
...
5. Do not use associated, correlated, secondary, or indirect findings to infer a target. In
   particular, effusion does not establish synovitis, and synovitis does not establish effusion.
...
10. If a report says 'muscle contusion' and also 'no bone contusion', assign Contusion=N and quote
    the direct bone-contusion denial.
11. When unsure about direct evidence or negation scope, use U only for a direct uncertain mention
    of the target; otherwise use M.
12. Do not add medical advice or explanations outside the requested JSON.
""".strip()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract direct-only 12-target evidence from 58 labeled reports using v5."
    )
    parser.add_argument("--input", default=str(PROJECT_DIR / "train.csv"))
    parser.add_argument(
        "--jsonl", default=str(EXTRACTION_DIR / "gemma4_e4b_extract_58_v5.jsonl")
    )
    parser.add_argument(
        "--long-csv", default=str(EXTRACTION_DIR / "gemma4_e4b_extract_58_v5_long.csv")
    )
    parser.add_argument(
        "--summary-csv",
        default=str(EXTRACTION_DIR / "gemma4_e4b_extract_58_v5_summary.csv"),
    )
    parser.add_argument(
        "--indices",
        default="",
        ...
        try:
            ...
            if attempt < attempts:
                retry_note = (
                    ...
                    "P/N/U require one exact report quote in e, and M requires an empty e. "
                    "Classify each target only from direct statements about that target."
                )
                time.sleep(2 * attempt)
...
def apply_v5_policy(extraction: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Keep every target strictly equal to its direct LLM judgment."""
        ...
        for line in handle:
            if line.strip():
                ...
                if item.get("prompt_version") != PROMPT_VERSION:
                    raise ValueError(
                        f"Existing JSONL contains {item.get('prompt_version')!r}, "
                        f"but this script requires {PROMPT_VERSION!r}: {path}"
                    )
                item["postprocessed_targets"] = apply_v5_policy(item["extraction"])
...
def write_tabular_outputs(records: list[dict[str, Any]], long_path: Path, summary_path: Path) -> None:
    ...
    for record in records:
        ...
        processed_targets = record.get("postprocessed_targets") or apply_v5_policy(
            record["extraction"]
        )
        for target in TARGETS:
...
def main() -> None:
    ...
    for position, (source_index, row) in enumerate(labeled.iterrows(), start=1):
        ...
        postprocessed_targets = apply_v5_policy(extraction)
        record = {
